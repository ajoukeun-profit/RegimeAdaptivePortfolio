...
## Phase 4 시도: 자산 확장 + 라벨 기간 연장 (음성 결과)

> 팀원이 논문 기반 ETF 23자산 데이터를 추가로 제공하여, 더 많은 자산과 더 긴 기간으로 성능 향상을 시도했다.

### 시도 배경

Phase 3까지 학습 샘플이 488개로 제한된 원인: SPY HMM 라벨이 2012년부터 시작  
→ SPY 원본 데이터를 2004년부터 확보하여 라벨을 재생성하고, 자산도 늘려 실험

### 실험 내용

**Step 1 — SPY HMM 라벨 재생성 (2004년 기준)**

```bash
python3 scripts/hmm_regime_labeling.py \
  --input data/raw/paper_etfs_daily/spy_daily.csv \
  --output data/processed/spy_hmm_regime_labels_5d_2004.csv \
  --train-window 504 --fit-step 5 --states 3 --smoothing-window 5
```

결과: 699개 → **1003개** 라벨 (2006-06-30 ~ 2026-05-27)

**Step 2 — 자산 확장 데이터셋 생성 및 학습**

| 데이터셋 | 자산 수 | 피처 수 | 샘플 수 | Train | 최고 Test Acc |
|---------|--------|--------|--------|-------|-------------|
| 기존 Phase 3 | 4자산 | 40 | 698 | 488 | **61.9%** |
| 20자산 (CPER 제외) | 20자산 | 200 | 844 | 590 | 55.9% |
| 15자산 (2004~ 시작 자산만) | 15자산 | 150 | 1002 | 701 | 60.0% |

- CPER 제외 이유: 2011년 상장으로 그 이전 라벨 303개 전부 스킵됨
- 15자산: AGG, EEM, EFA, EWJ, EWY, GLD, HYT, IEF, IEV, IJR, IWM, IYR, SPY, TIP, TLT

### 결론

샘플을 701개(+43%)로 늘렸지만 성능은 60.0%로 Phase 3(61.9%)를 넘지 못했다.

**원인 분석**:
1. **피처 차원이 같이 증가**: 40→150으로 늘어나 작은 모델이 처리하기 어려움 → 과적합
2. **자산 간 상관관계 높음**: 글로벌 ETF들은 시장 충격에 동반 반응하여, 추가 자산이 새로운 정보보다 중복 노이즈에 가까움
3. **샘플 수 증가의 한계**: 단순히 양적 증가보다 질적으로 다른 정보가 필요

**최종 결정**: Phase 3 (4자산 40피처, seed=42, AdamW, neutral-boost=1.2) 61.9% 유지
...
따라서 최종 단계에서는 국면 분류 모델은 유지하고, 확률을 포트폴리오 비중으로 바꾸는
리밸런싱 정책을 Regime Momentum Tilt로 확장했다.
그 결과 기존 Conv1D+LSTM SPY/Cash 전략의 누적수익률 21.9%를 43.1%로 개선했다."
```
