...
| **Phase 3** | **Cross-asset Baseline** | **488** | **40** | **62.9%** | **53.5%** | 19.0% | **95.1%** |
| Phase 3 | B: 모델 축소 + Dropout 0.7 | 488 | 40 | 53.3% | 37.2% | 0.0% | 97.6% |
| Phase 3 | C: Cosine Annealing LR | 488 | 40 | 49.5% | 20.9% | 19.0% | 95.1% |
| Phase 3 | D: Focal Loss γ=2 | 488 | 40 | 54.3% | 16.3% | **61.9%** | 90.2% |
| Phase 3 | E: Patience 100 | 488 | 40 | 49.5% | 25.6% | 19.0% | 90.2% |
| Phase 3 | F: LSTM 전체 시점 평균 | 488 | 40 | 49.5% | 44.2% | 9.5% | 75.6% |
| Phase 4 | 4-A: 3일 간격 라벨 | 814 | 10 | 26.3% | 20.6% | 86.5% | 0.0% |
| Phase 4 | 4-B: neutral-boost 1.2 | 488 | 40 | 61.9% | 60.5% | 0.0% | 95.1% |
| Phase 4 | 4-C: neutral-boost 2.0 | 488 | 40 | 52.4% | 32.6% | 52.4% | 73.2% |
| **Phase 4** | **4-D: 팀원 best_model** | **488** | **40** | **63.8%** | **65.1%** | 0.0% | **95.1%** |

**최종 선택 모델**: **팀원 best_model_mr.pt (Phase 4-D)**
- 전체 정확도 최고: **63.8%** (전체 실험 중 1위)
- Bear recall: **65.1%** (전체 실험 최고 — 하락장 인식 능력 최강)
...
## Phase 4: 팀원 코드 개선 + Neutral-boost 실험
...
### Phase 4-A: 3일 간격 라벨 실험

**가설**: 5거래일 간격 라벨(699개) → 3거래일 간격(1,164개)으로 샘플을 1.67배 늘린다.
...
python3 scripts/hmm_regime_labeling.py \
  ...
  --output data/processed/spy_hmm_regime_labels_3d.csv \
  --train-window 504 --fit-step 3 --states 3 \
  --smoothing-window 5 --target-horizon 1

# 데이터셋 생성 (훈련 샘플 488 → 814개)
python3 scripts/prepare_supervised_dataset.py \
  --raw data/raw/spy_daily.csv \
  --labels data/processed/spy_hmm_regime_labels_3d.csv \
  --output data/processed/spy_supervised_30d_3d.npz \
  --input-window 30 --target-horizon 1

# 학습
python3 scripts/train.py \
  --data data/processed/spy_supervised_30d_3d.npz \
  --model-output outputs/models/best_model_3d.pt \
...
**분석 및 실패 원인**: 샘플 수는 814개로 늘었지만, 30일 윈도우에서 3일 간격이면 인접 샘플이 27/30일을 공유한다. 이로 인해 validation 성능(bal_acc 46.6%)이 인위적으로 높게 나타나는 반면, 실제 test(미래 데이터)에서는 26.3%로 폭락하는 과적합이 발생했다. **샘플 수 증가가 독립적인 정보 증가를 의미하지 않는다.** 3일 간격은 사실상 동일 데이터를 3일씩 밀어서 복사한 것에 가깝다.

---

### Phase 4-B: Neutral-boost 1.2

**명령어**:
```bash
python3 scripts/train.py \
  --data data/processed/cross_asset_supervised_30d_5d.npz \
  --model-output outputs/models/best_model_cross_asset_v2.pt \
  --epochs 80 --patience 10 --batch-size 16 \
  --lr 1e-4 --conv-channels 16 --lstm-hidden 32 \
  --dropout 0.6 --weight-decay 1e-3 \
  --neutral-boost 1.2 --best-metric val_bal_acc --seed 42
...
**분析**: neutral_boost=1.2는 Neutral을 살리기에 충분하지 않았다. balanced accuracy 기준으로 best 모델을 저장했음에도 test에서 Neutral recall 0%가 나왔다. val 셋에 Neutral이 19개뿐이어서 best 선정 자체가 불안정하다.

---

### Phase 4-C: Neutral-boost 2.0

**가설**: neutral_boost 배수를 1.2 → 2.0으로 높여 Neutral 패널티를 강하게 부과한다.

**명령어**:
```bash
python3 scripts/train.py \
  --data data/processed/cross_asset_supervised_30d_5d.npz \
  --model-output outputs/models/best_model_cross_asset_v3.pt \
  --epochs 80 --patience 15 --batch-size 16 \
  --lr 1e-4 --conv-channels 16 --lstm-hidden 32 \
  --dropout 0.6 --weight-decay 1e-3 \
  --neutral-boost 2.0 --best-metric val_bal_acc --seed 42
```
...
**분析**: neutral_boost=2.0으로 Neutral recall이 0% → 52.4%로 크게 향상되었다. 그러나 전체 accuracy가 52.4%로 하락했고, Bear와 Bull recall도 함께 낮아졌다. Neutral을 살리면 다른 클래스가 희생되는 trade-off가 다시 확인되었다. **neutral_boost 1.2와 2.0 사이 어딘가에 최적점이 있을 가능성이 있다.**

---

### Phase 4-D: 팀원 best_model 평가

팀원이 동일한 cross-asset 데이터로 학습한 `best_model_mr.pt`를 받아 우리 test 셋으로 평가했다.

**명령어**:
```bash
python3 scripts/evaluate_model.py \
  --model outputs/models/best_model_mr.pt \
  --data data/processed/cross_asset_supervised_30d_5d.npz \
  --conv-channels 16 --lstm-hidden 32
```
...
**분析**: 전체 정확도 63.8%로 Phase 1~4 통틀어 최고 수치. Bear recall 65.1%도 역대 최고. 단, Neutral recall은 0%로 여전히 해결되지 않았다. 팀원이 추가 스케줄러 옵션(`--scheduler plateau --scheduler-factor 0.5 --scheduler-patience 3 --min-lr 1e-6`)을 활용한 버전으로 학습한 결과로 추정된다. **이 모델을 최종 모델로 채택한다.**
...
|------|---------|------|---------|------|------|
| Phase 3 Cross-asset Baseline | 62.9% | 53.5% | 19.0% | 95.1% | 이전 최고 |
| 4-A: 3일 간격 데이터 | 26.3% | 20.6% | 86.5% | 0.0% | 샘플 중복 과적합 |
| 4-B: neutral-boost 1.2 | 61.9% | 60.5% | 0.0% | 95.1% | boost 부족 |
| 4-C: neutral-boost 2.0 | 52.4% | 32.6% | 52.4% | 73.2% | boost 과도 |
| **4-D: 팀원 best_model** | **63.8%** | **65.1%** | 0.0% | **95.1%** | **최종 채택** |
...
SPY 테스트셋만으로 평가하면 61.9%로 소폭 향상되었고,
Bear recall은 58.8%로 전체 Phase 중 최고를 기록했다.

[Phase 3 — 교차 자산 피처 (Cross-asset Features)]
라벨 충돌 문제를 해결하기 위해, 4개 자산 피처를 하나의 입력으로 합치고
라벨은 SPY 국면만 사용하는 방식으로 전환했다.
입력 피처: 10 → 40개 (SPY+QQQ+GLD+TLT 각 10개 concat)
결과: 62.9%로 전체 11개 실험 중 최고 정확도 달성.

이후 과적합 해소를 위해 모델 축소, Focal Loss, Cosine Annealing을 순차 실험했으나
모두 baseline(62.9%)보다 낮았다. 데이터 488개라는 근본 제약 앞에서
모델/학습 전략 변경의 효과는 제한적이었다.
