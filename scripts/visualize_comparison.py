...
# 2022 bear에서 "DL Regime SPY/Cash" → bull에서도 같은 simple policy baseline
bear_keys = ["Buy & Hold", "EW (1/N)", "60/40", "MA Crossover",
             "DL Regime SPY/Cash", "Regime Momentum Tilt"]
...
def bar_colors(keys):
    return [BLUE if "DL Regime" in s else
...
# ── 범례 및 요약 텍스트 ──────────────────────────────────────────
lstm_patch = mpatches.Patch(color=BLUE,   label="DL Regime SPY/Cash")
...
# 핵심 인사이트 — 실제 수치 자동 계산
lstm_mdd_bear = abs(bear["DL Regime SPY/Cash"]["mdd"]) * 100
bnh_mdd_bear  = abs(bear["Buy & Hold"]["mdd"]) * 100
lstm_mdd_bull = abs(bull["DL Regime SPY/Cash"]["mdd"]) * 100
bnh_mdd_bull  = abs(bull["Buy & Hold"]["mdd"]) * 100
reduction     = (1 - lstm_mdd_bear / bnh_mdd_bear) * 100

insight = (
    f"하락장(2022): DL Regime SPY/Cash  MDD -{lstm_mdd_bear:.1f}%  vs  Buy&Hold -{bnh_mdd_bear:.1f}%  →  낙폭 {reduction:.0f}% 감소\n"
    f"상승장(2024~): DL Regime SPY/Cash  MDD -{lstm_mdd_bull:.1f}%  vs  Buy&Hold -{bnh_mdd_bull:.1f}%  →  꾸준한 위험 관리"
)
